...
void SphereWidget::setWidgetData(Surface const* _modleData)
{
	setConnect(false);
...
bool SphereWidget::getWidgetData(Surface * _modleData,
...
void SphereWidget::changeText(int index, const string& t)
{
	if (index < 7)
...
void SphereWidget::on_DefChange(QString t)
{
	bool ok = false;
	t.toFloat(&ok);
	string temp;
