...
void RectangleWidget::setWidgetData(Surface const* _modleData)
{
	setConnect(false);
...
bool RectangleWidget::getWidgetData(Surface * _modleData,
...
void RectangleWidget::changeText(int index, const string& t)
{
	if (index < 7)
...
void RectangleWidget::on_DefChange(QString t)
{
	bool ok = false;
	t.toFloat(&ok);
	string temp;
